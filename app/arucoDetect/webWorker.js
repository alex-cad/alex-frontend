...
async function detectAruco(event) {
    ...
    try {
        ...
        self.postMessage({ type: "JS_RESULT", deviceId, result: { rvecs, tvecs, num, markerId: markerIds.ucharPtr(0, 0)[0] } });
...
function getPoseData(event) {
    const { deviceId } = event.data;
    self.postMessage({ type: "POSE_DATA", deviceId, result: poseData });
}
